...
class Zombie(Entity):
    def __init__(self, position):
        ...
        self.health = 2
        self.walk_speed = 1
        self.player = player  # will be assigned after player is created
        self.is_alive = True
...
# When a zombie is close enough and its cooldown is finished, it deals 2 damage,
# flashes the red edges, and then that zombie waits 30 seconds before it can damage again.
def check_zombie_collision():
    ...
    for zombie in scene.entities:
        if isinstance(zombie, Zombie) and zombie.is_alive:
            ...
            if distance_to_player < 1.5 and current_time - zombie.last_damage_time >= 30:
                player_health = max(0, player_health - 2)
                # Update the health bar width proportionally (full width is 0.4)
